...
function select(p){selected=p;$('#pageImage').src=p.image;$('#pageImage').alt=p.title;$('#pageTitle').textContent=p.title;$('#pageSection').textContent=`${p.section} • ${p.label}`;$('#original').href=p.image;$('#ocr').textContent=p.ocr||'OCR unavailable';renderList();}
