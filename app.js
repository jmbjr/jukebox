...
function renderFigures(page){
  ...
  host.innerHTML=items.map(f=>{const callouts=[...f.callouts].sort((a,b)=>Number(a.number)-Number(b.number)),positioned=f.calloutsPositioned===true;return `<article class="figure-card">
    <div class="figure-heading"><div><p class="eyebrow">Extracted figure ${f.number}</p><h2>${f.title}</h2></div><small>From page ${f.source.pageLabel}${f.source.legendPageLabel?` · legend ${f.source.legendPageLabel}`:''}</small></div>
